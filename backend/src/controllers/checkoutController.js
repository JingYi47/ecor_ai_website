...
export const createCheckout = async (req, res) => {
  try {
    ...
    // ===== 10. Xóa các item đã chọn khỏi giỏ hàng =====
    const selectedIds = selectedItems.map((i) => i._id.toString());
    cart.items = cart.items.filter((i) => !selectedIds.includes(i._id.toString()));
    cart.discountApplied = 0;
    cart.coupon = null;
    // Tính lại subtotal/total sau khi xóa
    const remaining = cart.items.filter((i) => i.isSelected);
    cart.subtotal = remaining.reduce((s, i) => s + i.priceAtAddition * i.quantity, 0);
...
export const getOneCheckout = async (req, res) => {
  try {
    const checkout = await Checkout.findOne({
      _id: req.params.id,
      user: req.user._id,
    }).populate("items.product", "name images imageUrl").populate("coupon", "code name");
