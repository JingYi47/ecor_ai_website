...
export const applyCoupon = async (req, res) => {
  try {
    ...
    if (!coupon) return res.status(404).json({ success: false, message: "Mã giảm giá không hợp lệ hoặc đã hết hạn" });

    const now = new Date();
    if (coupon.startDate > now)
      return res.status(400).json({ success: false, message: "Mã chưa tới thời gian sử dụng" });
    ...
    return res.json({
      ...
      coupon: { code: coupon.code, name: coupon.name, discountType: coupon.discountType, discountValue: coupon.discountValue },
      discount,
    });
  } catch (err) {
