...
export const createMomoPayment = async (req, res) => {
  try {
    ...
    const extraData  = Buffer.from(JSON.stringify({ checkoutId: checkout._id })).toString("base64");
    ...
    res.status(500).json({ message: "Create MoMo payment failed", detail: err.message });
  }
};
