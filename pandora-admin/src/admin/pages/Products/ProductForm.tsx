...
import {
  ...
  Space, Switch, Upload, message, Tag,
} from "antd";
...
export function ProductForm({ mode, product }: { mode: "create" | "edit"; product?: (Product & any) | null }) {
  ...
  const onFinish = async (values: ProductFormValues) => {
    ...
    const colorsPayload = colors
      ...
      .map((c) => ({ name: c.name.trim(), stock: c.stock }));
    ...
    try {
      ...
      if (mode === "create") {
        ...
        if (!product) return;
        await adminProductApi.update(product._id, payload);
        savedId = product._id;
                    ...
                    <div style={{ marginBottom: 8 }}>
                      ...
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                        {color.existingImages.map((img, j) => (
                          <img key={j} src={img.url} width={60} height={60}
